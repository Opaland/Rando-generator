...
npm run listes     # deux listes qui disent la même règle, comparées
...
`npm run monkey`, `npm run mesure`, `npm run reel`, `npm run chargement` et
`npm run panneau` —, pas seulement la première : `playwright.config.ts` la
lit pour chacune.
...
| [`docs/PERSONAS.md`](./docs/PERSONAS.md)             | Dix-neuf personnes suivies pas à pas, et l'endroit exact où elles s'arrêtent                                                                                     |
